...
function switchSec(id,btn){document.querySelectorAll('.nav-pill').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.sec').forEach(s=>s.classList.remove('active'));btn.classList.add('active');document.getElementById(`sec-${id}`).classList.add('active');if(id==='analytics')loadAnalytics();if(id==='ai')loadAiSummary();}
...
async function loadAiSummary() {
    const el = document.getElementById('ai-auto-content');
    const dateEl = document.getElementById('ai-auto-date');
    const today = new Date().toLocaleDateString('en-CA', {timeZone: 'Asia/Kolkata'});
    const yest = new Date(Date.now() - 86400000).toLocaleDateString('en-CA', {timeZone: 'Asia/Kolkata'});

    try {
        const r = await fetch('https://boxcast-telemetry.boxboxcric.workers.dev/query', {
            method: 'POST',
            headers: {'Authorization': 'Bearer boxcast_secure_telemetry_key_2026', 'Content-Type': 'application/json'},
            body: JSON.stringify({
                query: "SELECT * FROM ai_summaries WHERE date_partition IN (?, ?) ORDER BY date_partition DESC LIMIT 1",
                args: [today, yest]
            })
        });
        ...
        if (d.data && d.data.length > 0) {
            const row = d.data[0];
            dateEl.textContent = row.date_partition;
            el.innerHTML = marked.parse(row.summary) + `<div class="mt-4 pt-3 border-t border-slate-800 text-[10px] text-slate-500">Generated by ${row.model} at ${row.generated_at}</div>`;
        } else {
            dateEl.textContent = today;
            el.innerHTML = '<div class="text-center text-slate-500 py-10"><i class="fa-solid fa-clock mb-3 text-2xl"></i><br>No summary generated yet for today.</div>';
        }
    } catch(e) {
        el.innerHTML = `<div class="text-red-400 p-4 bg-red-500/10 rounded-xl">Error loading summary: ${e.message}</div>`;
    }
}
...
async function sendAiQuery() {
    ...
    try {
        const cfg = await (await fetch('config.json')).json();
        const pat = decrypt(cfg.encrypted_token, dashPwd);
        if (!pat.startsWith('ghp_')) throw new Error('Bad key');
        ...
        if (resp.ok && data.choices) {
            ...
            histEl.innerHTML += `<div class="bg-slate-800/50 p-3 rounded-xl rounded-tl-sm text-slate-300 border border-slate-700/50 mr-10 mb-3 prose prose-invert prose-sm">${marked.parse(reply)}</div>`;
        } else {
            throw new Error(data.error?.message || 'API Error');
        }
    } catch(err) {
        document.getElementById(loadId)?.remove();
        histEl.innerHTML += `<div class="bg-red-500/10 p-3 rounded-xl rounded-tl-sm text-red-400 border border-red-500/30 mr-10 mb-3"><i class="fa-solid fa-triangle-exclamation"></i> Error: ${err.message}</div>`;
        if (err.message.includes('Bad key')) { dashPwd=''; updateAuthUI(); openAuth(); }
    }
    histEl.scrollTop = histEl.scrollHeight;
}
