...
/**
 * Metadata for a suggested genre or topic chip in tag editors and folder creation sheets.
 * Pairs the display name with its default matching icon from [GenreIcons].
 */
...
/**
 * Canonical catalog genres and popular topic tags with rich keyword dictionaries
 * for real-time fuzzy matching across podcast tags and subscription folders.
 */
...
private fun scoreSuggestion(
    ...
    return when {
        ...
        suggestion.iconKey?.contains(trimmed) == true -> 7
        else -> null
    }
}
 ...
 * When items have identical match scores, items present in the user's library (`isFromLibrary = true`)
 * are prioritized first.
 * When [query] is blank, returns [allSuggestions] in their provided order.
 */
fun filterGenreSuggestions(
    ...
    return allSuggestions
        ...
        .sortedWith(compareBy<Pair<GenreSuggestion, Int>> { it.second }.thenByDescending { it.first.isFromLibrary })
        .map { it.first }
}
...
fun findSuggestedIcons(
    ...
    val suggestionIconKeys = matchingSuggestions.mapNotNull { it.iconKey }.distinct()
...
/**
 * Prepares suggestions for folder creation by combining subscribed shows' library genres
 * (distinct tags/genres from the user's library) with standard catalog suggestions.
 * Library genres are placed at the top and marked with [GenreSuggestion.isFromLibrary] = true,
 * ensuring they appear first when the query is blank and take priority during keyword searches.
 */
fun buildFolderSuggestionsWithLibrary(
    ...
    val baseByName = baseSuggestions.associateBy { it.name.trim().lowercase() }
    val librarySuggestions = mutableListOf<GenreSuggestion>()
    val matchedBaseKeys = mutableSetOf<String>()

    libraryGenreNames.forEach { genreName ->
        val matchedBase = baseByName[genreName.lowercase()]
        if (matchedBase != null) {
 ...
 * displaying them as suggestions when tagging podcasts in [cx.aswin.boxlore.feature.info.components.PodcastGenreEditSheet].
 */
