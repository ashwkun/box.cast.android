...
- `FolderEditSheet` and `FolderEditComponents`: Subscription folder creation and edit sheet featuring a streamlined hierarchy, prominent name input with real-time keyword suggestions and preset icons (prioritizing subscribed library genres), visual 3×3 grid footprint diagrams for 7 display sizes up to max 3×3 (`COMPACT 1×1`, `WIDE 2×1`, `FEATURED 2×2`, `LARGE 2×3`, `SHELF 3×1`, `PANEL 3×2`, `SHOWCASE 3×3`), contextual 1×1 cover display selector (`[ Folder Icon ]` vs `[ Podcast Grid ]`), compact optional icon picker row, LazyRow-powered horizontal scrolling with crisp edge stops (overscroll bounce disabled to prevent stretch distortion and floaty spring delay), and auto-sync with genre tags.
